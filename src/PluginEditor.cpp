...
namespace
{
    // The GUI bridge shim, withBridge, toBytes and restoredValuesJson all live in
    // BridgeShim.h so every GUI-serving path (this legacy native editor, WebEditor
    // and LockedEditor) injects byte-identical JS. This file used to carry its own
    // copy, which drifted out of sync; use the shared ones instead.
    using vstai::shim::toBytes;
    using vstai::shim::withBridge;
    using vstai::shim::restoredValuesJson;
