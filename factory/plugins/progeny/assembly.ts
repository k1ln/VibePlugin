// =====================================================================
//  PROGENY — a single-oscillator semi-modular MONO synth built around an
//  8-step SEQUENCER (the tabletop that launched a modern semi-modular line).
//  One saw/pulse oscillator with variable pulse width plus a square SUB runs
//  into a warm 4-pole transistor-ladder low-pass with a snappy ADSR, a free
//  LFO, sample & hold, noise and a clocked step sequencer that keeps a held
//  note dancing. Signature = the PATCHBAY: a 5x4 modulation matrix
//  (LFO/Env/SEQ/S&H/Noise -> Pitch/Cutoff/PWM/Verb) whose routing amounts ARE
//  parameters. Normalled SEQ->Pitch and Env->Cutoff dance unpatched. Mono.
// =====================================================================
...
const NSRC: i32 = 5;
const NDST: i32 = 4;
const NUM_PARAMS: i32 = 42;
const P_STEP: i32 = 34;   // 34..41 : 8 editable sequencer step pitches (0.5 = no offset)
...
let seqClk: f32 = 0.0;
let seqVal: f32 = 0.0;
const seqTab: StaticArray<f32> = new StaticArray<f32>(8);
...
export function init(sr: f32, maxFrames: i32, numChannels: i32): void {
  ...
  env = 0.0; envSt = 0; gate = 0; curFreq = 220.0; tgtFreq = 220.0; noteHeld = -1; seqStep = 0; seqClk = 0.0; seqVal = 0.0;
  ...
  seqTab[0]=0.0; seqTab[1]=0.42; seqTab[2]=-0.25; seqTab[3]=0.58; seqTab[4]=0.0; seqTab[5]=0.72; seqTab[6]=-0.4; seqTab[7]=0.25;
  params[P_ROUTE + 4 * NDST + 0] = 0.55;   // normalled SEQ -> Pitch (src4)
  for (let i = 0; i < 8; i++) params[P_STEP + i] = 0.5 + seqTab[i] * 0.5;   // editable steps default to the pattern
}
...
export function process(n: i32): void {
  ...
  const sq: f32 = osc1N;                                  // saw(0)->pulse(1) morph
  ...
  const seqStepHz: f32 = 1.0 + lfoN * 11.0;   // 1..12 sequencer steps / sec
  ...
  for (let i = 0; i < n; i++) {
    ...
    seqClk += seqStepHz / sampleRate;
    if (seqClk >= 1.0) { seqClk -= 1.0; seqStep = (seqStep + 1) & 7; seqVal = (params[P_STEP + seqStep] - 0.5) * 2.0; }

    // ---- destination modulations (full 5x4 matrix; src4 = sequencer) ----
    const pitchMod: f32 = lfo*route(0,0) + env*route(1,0) + snhVal*route(2,0) + noise*route(3,0) + seqVal*route(4,0);
    const cutMod:   f32 = lfo*route(0,1) + env*route(1,1) + snhVal*route(2,1) + noise*route(3,1) + seqVal*route(4,1);
    const pwmMod:   f32 = lfo*route(0,2) + env*route(1,2) + snhVal*route(2,2) + noise*route(3,2) + seqVal*route(4,2);
    const verbMod:  f32 = lfo*route(0,3) + env*route(1,3) + snhVal*route(2,3) + noise*route(3,3) + seqVal*route(4,3);

    // ---- oscillators ----
    const fmod: f32 = f32(Mathf.pow(2.0, pitchMod * 2.0));   // +-2 oct range
    const f1: f32 = curFreq * fmod;
    ph1 += f1 / sampleRate; if (ph1 >= 1.0) ph1 -= 1.0;
    ph2 += (f1 * 0.5) / sampleRate; if (ph2 >= 1.0) ph2 -= 1.0;   // sub one octave down
    // pulse width: base from intN, modulated by the PWM destination
    let pw: f32 = clampf(0.15 + intN * 0.7 + pwmMod * 0.4, 0.05, 0.95);
    const saw1: f32 = ph1 * 2.0 - 1.0;
    const pul1: f32 = ph1 < pw ? 1.0 : -1.0;
    const o1: f32 = saw1 * (1.0 - sq) + pul1 * sq;
    const sub: f32 = ph2 < 0.5 ? 1.0 : -1.0;
    let oscmix: f32 = (o1 + sub * mixN * 0.8) * 0.62;
    ...
    // ---- spring reverb (comb + cascaded allpass, modulated a touch) ----
    const springAmt: f32 = clampf(verbN + verbMod * 0.5, 0.0, 1.0);
    const dl: f32 = springL[spIdx];
    const dr: f32 = springR[spIdx];
    let wL: f32 = sig + dl * springFb;
    let wR: f32 = sig * 0.92 + dr * springFb;
    // cascaded allpass for dispersion (the "boing")
    let aL: f32 = -0.6 * wL + apL0; apL0 = wL + 0.6 * aL;
    let aL2: f32 = -0.6 * aL + apL1; apL1 = aL + 0.6 * aL2;
    let aL3: f32 = -0.55 * aL2 + apL2; apL2 = aL2 + 0.55 * aL3;
    let aR: f32 = -0.6 * wR + apR0; apR0 = wR + 0.6 * aR;
    let aR2: f32 = -0.6 * aR + apR1; apR1 = aR + 0.6 * aR2;
    let aR3: f32 = -0.55 * aR2 + apR2; apR2 = aR2 + 0.55 * aR3;
    springL[spIdx] = clampf(aL3, -1.5, 1.5);
    springR[spIdx] = clampf(aR3, -1.5, 1.5);
    spIdx++; if (spIdx >= SPRINGN) spIdx = 0;

    let oL: f32 = sig * (1.0 - springAmt * 0.5) + aL3 * springAmt;
    let oR: f32 = sig * (1.0 - springAmt * 0.5) + aR3 * springAmt;
    oL = f32(Mathf.tanh(oL * out * 1.4));
    oR = f32(Mathf.tanh(oR * out * 1.4));
    outBuf[i] = oL;
    outBuf[MAX_FRAMES + i] = oR;
  }
}
