// =====================================================================
//  NUCLEON — a budget-champion 2-VCO semi-modular MONO synth with a fat,
//  detuned dual-oscillator core (a CEM-style saw partner), a resonant
//  low-pass and a hot OVERDRIVE output stage for thick, snarling bass and
//  leads. Snappy ADSR, free LFO, sample & hold and noise. Fully patchable:
//  a 5x4 modulation matrix (LFO/Env/S&H/Noise/Osc2 -> Pitch/Cutoff/PWM/
//  Overdrive) whose routing amounts ARE parameters. Normalled Env->Cutoff
//  + LFO->PWM. Mono, last-note. No host imports, no alloc in process().
// =====================================================================
...
export function process(n: i32): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const f2: f32 = f1 * ratio2 * 1.006;   // fine detune for a fat, beating unison
    ...
    // ---- overdrive output (dest3 -> extra drive) ----
    const odrv: f32 = 1.2 + clampf(verbN + verbMod * 0.7, 0.0, 1.6) * 3.0;
    let o: f32 = f32(Mathf.tanh(sig * out * odrv));
    o = o - o * o * o * 0.12;
    outBuf[i] = o;
