// =====================================================================
//  QUASAR — an "organismic" patchable MONO synth-voice for the
//  experimentally minded: a wild instrument that pushes back. Two
//  oscillators cross-modulate (through-zero FM) with a feedback path, so
//  small patch moves bloom into clangy, unpredictable, living textures. A
//  resonant ladder low-pass, snappy ADSR, free LFO, S&H and noise. Fully
//  patchable: a 5x4 matrix (LFO/Env/S&H/Noise/Osc2 -> Pitch/Cutoff/PWM/Chaos)
//  whose routing amounts ARE parameters. Normalled Env->Cutoff + LFO->PWM.
//  Not for straight subtractive sounds - for instruments that surprise you.
// =====================================================================
...
export function process(n: i32): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const chaos: f32 = clampf(verbN + verbMod * 0.6, 0.0, 1.5);
    const f1: f32 = curFreq * fmod * f32(Mathf.pow(2.0, lastO2 * chaos * 0.9));   // osc2 -> osc1 through-zero FM
    ...
    const inp: f32 = f32(Mathf.tanh((oscmix + fbk * chaos * 1.3) * drive));   // feedback bloom
    ...
    fbk = f32(Mathf.tanh(sig * (1.0 + chaos)));

    // ---- output ----
    let o: f32 = f32(Mathf.tanh(sig * out * 1.4));
    outBuf[i] = o;
