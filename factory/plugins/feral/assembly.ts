// =====================================================================
//  FERAL — a raw, aggressive 2-oscillator semi-modular MONO synth in the
//  Steiner-Parker lineage. Two oscillators (saw<->pulse, osc2 across an
//  interval) drive a snarling resonant multimode-ish low-pass, and the
//  signature is GRIND: a Brute-style feedback circuit that folds the output
//  back into the filter for dirt, honk and chaos. Snappy ADSR, free LFO,
//  sample & hold and noise round it out. Fully patchable: a 5x4 modulation
//  matrix (LFO/Env/S&H/Noise/Osc2 -> Pitch/Cutoff/PWM/Grind) whose routing
//  amounts ARE parameters. Normalled Env->Cutoff + a touch of LFO->PWM.
//  Mono, last-note priority. No host imports, no alloc in process().
// =====================================================================
...
export function init(sr: f32, maxFrames: i32, numChannels: i32): void {
  ...
  env = 0.0; envSt = 0; gate = 0; curFreq = 220.0; tgtFreq = 220.0; noteHeld = -1; lastO2 = 0.0; fbk = 0.0;
...
export function process(n: i32): void {
  ...
  const reso: f32 = resN * 4.6;                           // snarly, self-oscillating
  ...
  for (let i = 0; i < n; i++) {
    ...
    const inp: f32 = f32(Mathf.tanh((oscmix + fbk * clampf(verbN + verbMod*0.6,0.0,1.4) * 1.6) * drive));
    ...
    // Brute-style feedback GRIND: dest3 (verbMod) + the Grind knob (P_VERB)
    const grind: f32 = clampf(verbN + verbMod * 0.6, 0.0, 1.4);
    let sig: f32 = filtered * env;
    fbk = f32(Mathf.tanh(sig * 1.2));

    // ---- output: hard-driven, asymmetric bite ----
    let o: f32 = f32(Mathf.tanh(sig * out * (1.35 + grind * 1.4)));
    o = o + o * o * 0.06;                                   // touch of even-harmonic honk
    outBuf[i] = o;
