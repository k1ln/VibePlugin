// =====================================================================
//  CASCARA — a 3-oscillator semi-modular MONO synth that blends east and
//  west coast in one voice: three detuned oscillators (with a fifth-stacked
//  third) sum into a gentle WAVEFOLDER, then a warm 4-pole ladder low-pass,
//  so you can dial from clean subtractive to metallic folded timbre. Praised
//  for staying rock-solid in tune. Snappy ADSR, free LFO, S&H, noise. Fully
//  patchable: a 5x4 matrix (LFO/Env/S&H/Noise/Osc2 -> Pitch/Cutoff/PWM/Verb)
//  whose routing amounts ARE parameters. Normalled Env->Cutoff + LFO->PWM.
// =====================================================================
...
export function init(sr: f32, maxFrames: i32, numChannels: i32): void {
  ...
  ph1 = 0.0; ph2 = 0.0; ph3 = 0.0; lfoPh = 0.0; snhVal = 0.0; snhClk = 0.0;
...
export function process(n: i32): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    lastO2 = o2;
    ph3 += (f1 * 1.4983) / sampleRate; if (ph3 >= 1.0) ph3 -= 1.0;   // osc3 stacked a fifth up
    const o3: f32 = ph3 * 2.0 - 1.0;
    let osum: f32 = (o1 + o2 + o3 * 0.7) * 0.42;
    // West-coast fold blended in by mixN (0 clean -> 1 folded)
    let oscmix: f32 = osum * (1.0 - mixN) + f32(Mathf.sin(osum * (1.0 + mixN * 3.5) * 1.5708)) * mixN;
