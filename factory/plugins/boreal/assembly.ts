// =====================================================================
//  BOREAL — a big, aggressive 3-oscillator semi-modular MONO synth for
//  people who find typical semi-modulars too tame. Three detuned oscillators
//  (a fat saw stack with a sub) drive a snarling resonant ladder low-pass
//  with hard drive and a built-in spring reverb for size. Snappy ADSR, free
//  LFO, S&H, noise. Fully patchable: a 5x4 matrix (LFO/Env/S&H/Noise/Osc2 ->
//  Pitch/Cutoff/PWM/Verb) whose routing amounts ARE parameters. Normalled
//  Env->Cutoff + LFO->PWM. Mono, last-note priority.
// =====================================================================
...
export function process(n: i32): void {
  ...
  const reso: f32 = resN * 4.5;                           // aggressive, self-oscillating
  ...
  for (let i = 0; i < n; i++) {
    ...
    lastO2 = o2;
    ph3 += (f1 * 0.5017) / sampleRate; if (ph3 >= 1.0) ph3 -= 1.0;   // detuned sub-octave stack
    const o3: f32 = ph3 * 2.0 - 1.0;
    let oscmix: f32 = (o1 + o2 * (0.55 + mixN * 0.45) + o3 * 0.85) * 0.4;
