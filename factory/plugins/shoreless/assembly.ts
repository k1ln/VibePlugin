// =====================================================================
//  SHORELESS — a "no-coast" tabletop MONO synth that borrows from both
//  east and west without committing to either. A single sine/triangle core
//  runs into a smooth WAVEFOLDER (the west-coast overtone move) then a
//  gentle low-pass gate, so timbre grows from folding rather than
//  subtraction. A slope-style ADSR, free LFO, random source and noise feed
//  a 5x4 patch matrix (LFO/Env/Rand/S&H/Osc2 -> Pitch/Cutoff/Fold/Air)
//  whose routing amounts ARE parameters. Small, deep, experimental. Mono.
// =====================================================================
...
export function process(n: i32): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let pw: f32 = 0.5;   // (unused for the folded core)
    const sine1: f32 = f32(Mathf.sin(ph1 * TAU));
    const tri1: f32 = 1.0 - 4.0 * f32(Mathf.abs(ph1 - 0.5));
    let core: f32 = sine1 * (1.0 - sq) + tri1 * sq;        // sine -> triangle by osc1N
    lastO2 = f32(Mathf.sin(ph2 * TAU));                    // osc2 = a sine mod partner
    // WAVEFOLDER: fold amount from the Fold knob (mixN) + the Fold dest (pwmMod)
    const foldAmt: f32 = 1.0 + clampf(mixN + pwmMod * 0.6, 0.0, 1.6) * 6.0;
    let folded: f32 = f32(Mathf.sin(core * foldAmt * 1.5708));
    folded = folded * 0.8 + f32(Mathf.sin(folded * (1.0 + mixN * 2.0) * 1.5708)) * 0.2;
    let oscmix: f32 = folded * 0.62;
