// =====================================================================
//  DYNASTY — a lush, four-oscillator paraphonic-voiced semi-modular MONO
//  synth with a proper STEREO analog-style ping-pong delay: the flagship of
//  the range. Four detuned oscillators stack into a huge unison, run through
//  a warm 4-pole ladder low-pass with a snappy ADSR, then a wide stereo
//  delay for size and motion. Free LFO, S&H, noise. Fully patchable: a 5x4
//  matrix (LFO/Env/S&H/Noise/Osc2 -> Pitch/Cutoff/PWM/Delay) whose routing
//  amounts ARE parameters. Normalled Env->Cutoff + LFO->PWM. Last-note.
// =====================================================================
...
let spIdx: i32 = 0;
const DN: i32 = 14400;
const delayL: StaticArray<f32> = new StaticArray<f32>(DN);
const delayR: StaticArray<f32> = new StaticArray<f32>(DN);
let dIdx: i32 = 0;
...
export function init(sr: f32, maxFrames: i32, numChannels: i32): void {
  ...
  ph1 = 0.0; ph2 = 0.0; ph3 = 0.0; ph4 = 0.0; lfoPh = 0.0; snhVal = 0.0; snhClk = 0.0;
  ...
  for (let i = 0; i < DN; i++) { delayL[i] = 0.0; delayR[i] = 0.0; } dIdx = 0;
  // panel defaults
...
export function process(n: i32): void {
  ...
  for (let i = 0; i < n; i++) {
    ...
    lastO2 = o2;
    ph3 += (f1 * 0.994) / sampleRate; if (ph3 >= 1.0) ph3 -= 1.0;
    ph4 += (f1 * 1.0083) / sampleRate; if (ph4 >= 1.0) ph4 -= 1.0;
    const o3: f32 = ph3 * 2.0 - 1.0;
    const o4: f32 = ph4 * 2.0 - 1.0;
    let oscmix: f32 = (o1 + o2 + (o3 + o4) * (0.5 + mixN * 0.5)) * 0.3;
    ...
    // ---- stereo ping-pong delay (dest3 -> delay send) ----
    const dlyMix: f32 = clampf(verbN + verbMod * 0.5, 0.0, 1.0);
    const dfb: f32 = 0.4 + verbN * 0.35;
    const rdL: f32 = delayL[dIdx]; const rdR: f32 = delayR[dIdx];
    delayL[dIdx] = clampf(sig + rdR * dfb, -1.6, 1.6);
    delayR[dIdx] = clampf(sig * 0.95 + rdL * dfb, -1.6, 1.6);
    dIdx++; if (dIdx >= DN) dIdx = 0;
    let oL: f32 = f32(Mathf.tanh((sig * (1.0 - dlyMix * 0.55) + rdL * dlyMix) * out * 1.3));
    let oR: f32 = f32(Mathf.tanh((sig * (1.0 - dlyMix * 0.55) + rdR * dlyMix) * out * 1.3));
    outBuf[i] = oL;
